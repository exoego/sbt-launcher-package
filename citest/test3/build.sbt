...
lazy val root = (project in file("."))
  .settings(
    ...
    name := "Hello",
    check := {
      val xs = IO.readLine(s"..${File.separator}output.txt").toVector
      ...
      val ys = IO.readLines(s"..${File.separator}err.txt").toVector.distinct

      assert(ys.size == 1, s"ys has more than one item: $ys")
      assert(ys(0) startsWith "Java HotSpot(TM) 64-Bit Server VM warning", s"ys does not starts with Java HotSpot:\n$ys")
    }
  )
